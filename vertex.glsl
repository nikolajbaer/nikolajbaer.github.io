...
void main(){
    ...
    float pos = position.x / (width/2.); // position of this vertex from 1.0 to -1.0
    if(abs(pos) > 1. ){ pos = pos/pos; } // force to be max -1/1
    float zpos = ((position.z+(width/2.))/width); // current vertex position in wave  0.0 to 1.0

    // the x position of the wave
    float tpos = (0.5 - mod(time_val,period)/period) * 2.; // -1 to 1 for wave position
    ...
    float breakpos = (tpos>0.) ? cos(pos * PI) + 1. : 2.;
    ...
    float focusing = pow(abs(position.z/width),2.);// * distFromReef; // * (1. - (y_pos_base / amplitude));
    focusing = 0.;//focusing * abs(min(pos,0.)); // Disable focusing for now
    float t1 = (travel_width/2.)  - mod( (time_val+focusing) * (period/2.),travel_width);
    float y_pos = max(wave_y(position.x,t1) * pump, 0.); 


    // calculate vertical component as sine wave, amplified later on by v
    float height_above_reef = (y_pos - reef_location.y)/(amplitude*2.) * breakpos; 

    // twist based on height of wave
    // but scale influence down by xz distance from reef point
    float twist = pow(height_above_reef,2.); // twist based on height of wave
    float theta = twist * PI/4.;
    float x_pos = position.x;
    float y_twist = y_pos * cos(theta) + x_pos * sin(theta);
    float x_twist = x_pos * cos(theta) - y_pos * sin(theta);

    float twist_amp = 1.;
    float y = clamp((y_twist * twist_amp ),0.,1000.0);
    //float y = (y_twist * twist_amp );
    ...
    vpos = vec3(pos,y/amplitude, zpos); // pass current x/height to fragment shader
}